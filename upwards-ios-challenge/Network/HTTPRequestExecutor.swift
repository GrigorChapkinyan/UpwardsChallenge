...
fileprivate let general = OSLog(subsystem: Constants.HTTPRequestExecutor.Util.logSubsystem.rawValue, category: Constants.HTTPRequestExecutor.Util.general.rawValue)

/// Executor for HTTP requests
final class HTTPRequestExecutor: NSObject, IRequestExecutor, URLSessionDelegate {
    // MARK: - Private Properties
    
    private let urlSessionConfig: URLSessionConfiguration?
    private lazy var urlSession: URLSession = {
        return (urlSessionConfig != nil) ? URLSession(configuration: urlSessionConfig!, delegate: self, delegateQueue: nil) : URLSession.shared
    }()
    
    // MARK: - Initializers

    init(urlSessionConfig: URLSessionConfiguration? = nil) {
    ...
    func execute(_ request: IRequest) async -> Result<Any, Error> {
        do {
            ...
            // Trying to get HTTP status code from URLReponse
            guard let httpStatusCode = (urlResponse as? HTTPURLResponse)?.status else {
                throw HTTPError.noHttpResponse
            }
                    
            // Checking for the errors from the HTTP status code
            if let httpError = HTTPError(with: httpStatusCode)  {
                throw httpError
            }
            
            // Checking "data" existance conditions
            if (data.isEmpty && !httpStatusCode.emptyBodyIsAllowed()) {
                throw HTTPError.invalidResponseBody
            }
    ...
    private func addLog(_ request: IRequest) {
        os_log("%s", log: general, type: .debug, request.description)
    }
}

// MARK: - Constants + NetworkManager

fileprivate extension Constants {
    /// Represents constants for "HTTPRequestExecutor" type
    struct HTTPRequestExecutor {
        /// Represents constants for util hardcodes
        enum Util: String {
            case logSubsystem = "com.upwards.challenge"
            case general = "general"
        }
    }
}

// MARK: - HTTPError + HTTPStatusCode

fileprivate extension HTTPError {
    /// Initializes an instance if passed "httpStatusCode" is from the error status code range, otherwise returns "nil"
    /// - Parameter httpStatusCode: Received HTTP response status code
    init?(with httpStatusCode: HTTPStatusCode?) {
...
fileprivate enum HTTPStatusCode: Int, Error {
    ...
    /// Indicates whether a empty body is allowed in HTTP response data
    /// - Returns: True if allowed, false otherwise
    func emptyBodyIsAllowed() -> Bool {
        return (responseType == .informational) || (self == .noContent) || (self == .notModified)
    }
}

// MARK: - HTTPURLResponse + HTTPStatusCode

fileprivate extension HTTPURLResponse {
    /// Creates "HTTPStatusCode" from current instance
